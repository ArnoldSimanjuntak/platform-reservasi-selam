import MapWrapper from "@/components/MapWrapper";
import type { Metadata } from "next";

export const metadata: Metadata = {
    title: "Peta Spot Selam - SulutDive Lembeh",
    description: "Jelajahi spot selam terbaik di Selat Lembeh melalui peta interaktif.",
};

export default function LokasiPage() {
    return (
        <main className="min-h-screen pt-24 pb-10 bg-gray-50">
            <div className="container mx-auto px-4 max-w-6xl">
                <div className="mb-8 text-center">
                    <h1 className="text-3xl md:text-4xl font-bold text-deepSea mb-3">
                        Peta Spot Selam Lembeh
                    </h1>
                    <p className="text-gray-600 max-w-2xl mx-auto">
                        Tap marker untuk melihat informasi spot dan memesan kapal langsung dari peta.
                    </p>
                </div>

                {/* Map Container */}
                <div className="w-full h-[70vh] min-h-[450px] rounded-2xl overflow-hidden shadow-xl border-4 border-white">
                    <MapWrapper />
                </div>

                {/* Zone Legend */}
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-center">
                    <div className="p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                        <span className="font-bold text-[#023E8A] block mb-1">Zona 1 (Dekat)</span>
                        <span className="text-gray-500">Tanpa biaya tambahan.</span>
                    </div>
                    <div className="p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                        <span className="font-bold text-[#023E8A] block mb-1">Zona 2 (Menengah)</span>
                        <span className="text-gray-500">+ Rp 150.000 / trip.</span>
                    </div>
                    <div className="p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                        <span className="font-bold text-[#023E8A] block mb-1">Zona 3 (Jauh)</span>
                        <span className="text-gray-500">+ Rp 300.000 / trip.</span>
                    </div>
                </div>
            </div>
        </main>
    );
}
